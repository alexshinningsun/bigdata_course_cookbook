...
dir_paths = ["#{HOMEDIR}/bigdata_course_cookbook/results", "#{HOMEDIR}/bigdata_course_cookbook/results/cpu-measurement", "#{HOMEDIR}/bigdata_course_cookbook/results/memory-measurement", "#{HOMEDIR}/bigdata_course_cookbook/results/ioping"]
...
file '#{HOMEDIR}/bigdata_course_cookbook/results/ioping/*' do
  action :delete
  backup false
end
...
search("aws_opsworks_instance").each do |instance|
  if "#{instance['hostname']}" != "#{my_instance['hostname']}"
    ...
    execute "Copy ioping result files from '#{instance['hostname']}' ec2 to workstation" do
      command "scp -o StrictHostKeyChecking=no -v -i #{HOMEDIR}/keys/#{key_file_name} ec2-user@#{instance['private_ip']}:#{TARGET_HOMEDIR}/ec2-testing-script/ioping/result-*  #{HOMEDIR}/bigdata_course_cookbook/results/ioping/"
    end
